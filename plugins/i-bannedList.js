...
╰────
`.trim()
    conn.sendButtonimg(m.chat, await(await fetch(fla + 'https://telegra.ph/file/96cc47cedf00f91da4598.jpg `${command}`)).buffer(), caption, wm, `Menu`, `${usedPrefix}menu`, m, { contextInfo: { mentionedJid: conn.parseMention(caption) }, mentions: await conn.parseMention(caption) })
}
